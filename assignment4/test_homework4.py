...
def plot_example_serial_and_parallel(Nt=100):
    ...
    u_parallel_0 = example_parallel_solution(dx, N, dt, 100, comm)
    #u_parallel_1 = example_parallel_solution(dx, N, dt, 400, comm)
    #u_parallel_2 = example_parallel_solution(dx, N, dt, 800, comm)
    #u_parallel_3 = example_parallel_solution(dx, N, dt, 1200, comm)
    ...
    if (comm.rank == 0):
        ...
        # solve in serial
        u_serial_0 = heat_serial(u_initial, dx, N, dt, 100)
        #u_serial_1 = heat_serial(u_initial, dx, N, dt, 400)
        #u_serial_2 = heat_serial(u_initial, dx, N, dt, 800)
        #u_serial_3 = heat_serial(u_initial, dx, N, dt, 1200)

        # plot results
        plt.clf()
        plt.plot(u_initial,'k--', linewidth=3, label='Initial Condition')
        plt.plot(u_serial_0, 'r--', linewidth=3, label='Serial Solution')
        #plt.plot(u_serial_1, 'r--', linewidth=3)
        #plt.plot(u_serial_2, 'r--', linewidth=3)
        #plt.plot(u_serial_3, 'r--', linewidth=3)
        
        plt.plot(u_parallel_0, 'r-', label='Parallel Solution')
        #plt.plot(u_parallel_1, 'r-')
        #plt.plot(u_parallel_2, 'r-')
        #plt.plot(u_parallel_3, 'r-')
        plt.legend()
        plt.title('Numerical Instability : dt = 0.51*dx**2', fontsize=14)
        plt.xlabel('$x$', fontsize=20)
        plt.ylabel('$u$', fontsize=20)


        plt.savefig('./instability_dxis_51.png')
...
if __name__ == '__main__':
    ...
    #plot_example_serial_and_parallel(Nt=100)
    #plot_example_serial_and_parallel(Nt=200)
    #plot_example_serial_and_parallel(Nt=300)
    #plot_example_serial_and_parallel(Nt=400)
